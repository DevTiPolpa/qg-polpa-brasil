...
type SqlInput = { name: string; type: any; value: unknown }
...
function buildWhere(f: FiltrosGlobais, alias = 'fv'): { clause: string; inputs: SqlInput[] } {
  const parts: string[] = []
  const inputs: SqlInput[] = []

  const addIn = (col: string, vals: string[], prefix: string) => {
    if (!vals.length) return
    const params = vals.map((v, i) => {
      const pname = `${prefix}${i}`
      inputs.push({ name: pname, type: sql.NVarChar(100), value: v })
      return `@${pname}`
    })
  ...
  if (f.dataInicio) {
    inputs.push({ name: 'dtIni', type: sql.Date, value: f.dataInicio })
    parts.push(`${alias}.dt_entrega_cliente >= @dtIni`)
  }
  if (f.dataFim) {
    inputs.push({ name: 'dtFim', type: sql.Date, value: f.dataFim })
    parts.push(`${alias}.dt_entrega_cliente <= @dtFim`)
  }
  if (f.codParc) {
    inputs.push({ name: 'codParc', type: sql.Int, value: f.codParc })
    parts.push(`${alias}.cod_parc = @codParc`)
  }
  if (f.codProduto) {
    inputs.push({ name: 'codProduto', type: sql.BigInt, value: f.codProduto })
    parts.push(`${alias}.cod_produto = @codProduto`)
  }
  if (f.uf) {
    inputs.push({ name: 'uf', type: sql.NVarChar(2), value: f.uf })
...
function shiftYear(iso?: string, delta = -1): string | undefined {
...
export async function getUserByEmail(email: string) {
  const rows = await query<{ id: number; name: string; email: string; password_hash: string; role: string; ativo: boolean; must_change_password: boolean; last_signed_in: Date | null }>(
    `SELECT id, name, email, password_hash, role, ativo, must_change_password, last_signed_in FROM users WHERE email = @email`,
...
export async function getUserById(id: number) {
  const rows = await query<{ id: number; name: string; email: string; password_hash: string; role: string; ativo: boolean; must_change_password: boolean }>(
    `SELECT id, name, email, password_hash, role, ativo, must_change_password FROM users WHERE id = @id`,
...
export async function updateLastSignedIn(id: number) {
  const p = await getPool()
  if (!p) return
  await p.request().input('id', sql.Int, id).query(`UPDATE users SET last_signed_in = GETDATE() WHERE id = @id`)
}
...
export async function listUsers() {
  return query<{ id: number; name: string; email: string; role: string; ativo: boolean; must_change_password: boolean; created_at: Date; last_signed_in: Date | null }>(
    `SELECT id, name, email, role, ativo, must_change_password, created_at, last_signed_in FROM users ORDER BY name`
  )
}

export async function createUser(data: { name: string; email: string; passwordHash: string; role: 'ADMIN' | 'VENDEDOR' }) {
  const p = await getPool()
  if (!p) throw new Error('DB unavailable')
  const res = await p.request()
    .input('name', sql.NVarChar(255), data.name)
    .input('email', sql.NVarChar(320), data.email)
    .input('hash', sql.NVarChar(255), data.passwordHash)
...
export async function updateUser(id: number, data: { name?: string; role?: string; ativo?: boolean }) {
  const p = await getPool()
  if (!p) return
  const sets: string[] = ['updated_at = GETDATE()']
  const req = p.request().input('id', sql.Int, id)
  if (data.name !== undefined) { req.input('name', sql.NVarChar(255), data.name); sets.push('name = @name') }
  if (data.role !== undefined) { req.input('role', sql.NVarChar(20), data.role); sets.push('role = @role') }
  if (data.ativo !== undefined) { req.input('ativo', sql.Bit, data.ativo ? 1 : 0); sets.push('ativo = @ativo') }
  await req.query(`UPDATE users SET ${sets.join(', ')} WHERE id = @id`)
}

export async function resetUserPassword(id: number, hash: string) {
  const p = await getPool()
  if (!p) return
  await p.request()
    .input('id', sql.Int, id)
...
// ─── Dashboard KPIs ───────────────────────────────────────────────────────────
export async function getKpisGlobais(f: FiltrosGlobais) {
  const { clause, inputs } = buildWhere(f)
  const rows = await query<{
    faturamentoBruto: number; volumeBruto: number
    faturamentoDevolucao: number; volumeDevolucao: number
    clientesUnicos: number; produtosUnicos: number; totalRegistros: number
  }>(`
    SELECT
...
export async function getKpisAnoAnterior(f: FiltrosGlobais) {
  return getKpisGlobais({ ...f, dataInicio: shiftYear(f.dataInicio), dataFim: shiftYear(f.dataFim) })
}

function buildOrcamentoWhere(f?: FiltrosGlobais): { clause: string; inputs: SqlInput[] } {
  const parts: string[] = []
  const inputs: SqlInput[] = []
  if (f?.dataInicio) { inputs.push({ name: 'dtIni', type: sql.Date, value: f.dataInicio }); parts.push('dt_prev_entrega_embarque >= @dtIni') }
  if (f?.dataFim)    { inputs.push({ name: 'dtFim', type: sql.Date, value: f.dataFim });    parts.push('dt_prev_entrega_embarque <= @dtFim') }
  if (f?.projetos?.length) {
    parts.push(`projeto IN (${f.projetos.map((_, i) => `@oproj${i}`).join(',')})`)
    f.projetos.forEach((p, i) => inputs.push({ name: `oproj${i}`, type: sql.NVarChar(100), value: p }))
  }
  if (f?.mercados?.length) {
    parts.push(`mercado_vendas IN (${f.mercados.map((_, i) => `@omerc${i}`).join(',')})`)
    f.mercados.forEach((m, i) => inputs.push({ name: `omerc${i}`, type: sql.NVarChar(100), value: m }))
  }
  if (f?.gruposProduto?.length) {
    parts.push(`grupo_produto IN (${f.gruposProduto.map((_, i) => `@ogrp${i}`).join(',')})`)
    f.gruposProduto.forEach((g, i) => inputs.push({ name: `ogrp${i}`, type: sql.NVarChar(100), value: g }))
  }
  return { clause: parts.length ? 'WHERE ' + parts.join(' AND ') : '', inputs }
}

export async function getOrcamentoKpis(f?: FiltrosGlobais) {
  const { clause, inputs } = buildOrcamentoWhere(f)
  const rows = await query<{ faturamentoTotal: number; volumeTotal: number; totalRegistros: number; clientesUnicos: number; produtosUnicos: number }>(
    `SELECT COALESCE(SUM(valor_pendente),0) AS faturamentoTotal, COALESCE(SUM(qtd_pendente_kg),0) AS volumeTotal,
...
export async function getOrcamentoMensal(f?: FiltrosGlobais) {
  const { clause, inputs } = buildOrcamentoWhere(f)
  const rows = await query<{ mes: string; faturamento: number; volume: number }>(
    `SELECT FORMAT(dt_prev_entrega_embarque,'yyyy-MM') AS mes,
...
interface FiltrosRecorrentes {
  dataInicio?: string; dataFim?: string
  mercados?: string[]; vendedores?: string[]; codParc?: number
}

function buildRecRealWhere(f: FiltrosRecorrentes): { conds: string[]; inputs: SqlInput[] } {
  const inputs: SqlInput[] = []
  const conds: string[] = [
  ...
  if (f.dataInicio) { inputs.push({ name: 'rdtIni', type: sql.Date, value: f.dataInicio }); conds.push('fv.dt_entrega_cliente >= @rdtIni') }
  if (f.dataFim)    { inputs.push({ name: 'rdtFim', type: sql.Date, value: f.dataFim });    conds.push('fv.dt_entrega_cliente <= @rdtFim') }
  if (f.mercados?.length)   { f.mercados.forEach((m, i)  => { inputs.push({ name: `rmerc${i}`,  type: sql.NVarChar(100), value: m }); }) ; conds.push(`fv.mercado_vendas IN (${f.mercados.map((_,i)=>`@rmerc${i}`).join(',')})`) }
  if (f.vendedores?.length) { f.vendedores.forEach((v, i) => { inputs.push({ name: `rvend${i}`, type: sql.NVarChar(100), value: v }); }) ; conds.push(`fv.nome_vendedor IN (${f.vendedores.map((_,i)=>`@rvend${i}`).join(',')})`) }
  if (f.codParc != null)    { inputs.push({ name: 'rcodParc', type: sql.Int, value: f.codParc }); conds.push('fv.cod_parc = @rcodParc') }
  return { conds, inputs }
}

function buildRecOrcWhere(f: FiltrosRecorrentes): { conds: string[]; inputs: SqlInput[] } {
  const inputs: SqlInput[] = []
  const conds: string[] = ["o.projeto = 'RECORRENTES'"]
  if (f.dataInicio) { inputs.push({ name: 'odtIni', type: sql.Date, value: f.dataInicio }); conds.push('o.dt_prev_entrega_embarque >= @odtIni') }
  if (f.dataFim)    { inputs.push({ name: 'odtFim', type: sql.Date, value: f.dataFim });    conds.push('o.dt_prev_entrega_embarque <= @odtFim') }
  if (f.mercados?.length) { f.mercados.forEach((m, i) => { inputs.push({ name: `omerc${i}`, type: sql.NVarChar(100), value: m }); }) ; conds.push(`o.mercado_vendas IN (${f.mercados.map((_,i)=>`@omerc${i}`).join(',')})`) }
  if (f.codParc != null) { inputs.push({ name: 'ocodParc', type: sql.Int, value: f.codParc }); conds.push('o.cod_parc = @ocodParc') }
  return { conds, inputs }
}

export async function getRecorrentesKpis(f: FiltrosRecorrentes) {
  const real = buildRecRealWhere(f)
  const orc  = buildRecOrcWhere(f)
  const [realRows, orcRows] = await Promise.all([
    query<{ fatAtual: number; volAtual: number }>(`
    ...
    `, real.inputs),
    query<{ orcVal: number; orcKg: number }>(`
...
export async function getRecorrentesTabela(f: FiltrosRecorrentes) {
  const real = buildRecRealWhere(f)
  const orc  = buildRecOrcWhere(f)
  return query<{
    codParc: number; razaoSocial: string
    volAtual: number; orcKg: number; fatAtual: number; orcVal: number
  }>(`
    WITH real_data AS (
      SELECT fv.cod_parc,
...
export async function getRecorrentesProdutos(codParc: number, f: FiltrosRecorrentes) {
  const realF = { ...f, codParc }
  const orcF  = { ...f, codParc }
  const real = buildRecRealWhere(realF)
  const orc  = buildRecOrcWhere(orcF)
  return query<{
    codProduto: number; nomeProduto: string
    volAtual: number; orcKg: number; fatAtual: number; orcVal: number
  }>(`
    WITH real_p AS (
...
export async function getRecorrentesFiltros() {
  const [vendedores, mercados] = await Promise.all([
    query<{ nome: string }>(`SELECT DISTINCT nome_vendedor AS nome FROM fato_vendas WHERE projeto = 'RECORRENTES' AND nome_vendedor IS NOT NULL ORDER BY nome_vendedor`),
    query<{ nome: string }>(`SELECT DISTINCT mercado_vendas AS nome FROM fato_vendas WHERE projeto = 'RECORRENTES' AND mercado_vendas IS NOT NULL ORDER BY mercado_vendas`),
  ])
  return { vendedores: vendedores.map(r => r.nome), mercados: mercados.map(r => r.nome) }
}

export async function getEvolucaoMensal(f: FiltrosGlobais) {
  const { clause, inputs } = buildWhere(f)
  return query<{ mes: string; faturamento: number; volume: number; vendaFirme: number; forecast: number; novoProjeto: number }>(`
...
export async function getEvolucaoMensalAnoAnterior(f: FiltrosGlobais) {
...
export async function getKpisPorTipoReceita(f: FiltrosGlobais) {
  ...
  return query<{ tipoReceita: string; faturamento: number; volume: number; clientes: number; registros: number }>(`
    SELECT
...
// ─── Vendedores ───────────────────────────────────────────────────────────────
export async function getPerformanceVendedores(f: FiltrosGlobais) {
  const { clause, inputs } = buildWhere(f)
  return query<{ nomeVendedor: string; faturamento: number; volume: number; clientes: number; produtos: number; vendaFirme: number; forecast: number; novoProjeto: number; fatNovoProjeto: number; fatRecorrente: number }>(`
...
export async function getEvolucaoVendedor(vendedor: string, f: FiltrosGlobais) {
  const parts: string[] = ['fv.nome_vendedor = @vend', '(fv.cod_top IS NULL OR fv.cod_top != 1023)', "(fv.[top] IS NULL OR fv.[top] NOT LIKE '%ESTOQUE MINIM%')"]
  const inputs: SqlInput[] = [{ name: 'vend', type: sql.NVarChar(100), value: vendedor }]
  if (f.dataInicio) { inputs.push({ name: 'dtIni', type: sql.Date, value: f.dataInicio }); parts.push('fv.dt_entrega_cliente >= @dtIni') }
  if (f.dataFim) { inputs.push({ name: 'dtFim', type: sql.Date, value: f.dataFim }); parts.push('fv.dt_entrega_cliente <= @dtFim') }
  const clause = 'WHERE ' + parts.join(' AND ')
  return query<{ mes: string; faturamento: number; volume: number; clientes: number }>(`
    SELECT FORMAT(fv.dt_entrega_cliente,'yyyy-MM') AS mes,
      COALESCE(SUM(fv.valor_pendente),0) AS faturamento,
      COALESCE(SUM(fv.qtd_pendente_kg),0) AS volume,
      COUNT(DISTINCT fv.cod_parc) AS clientes
...
export async function getClientesVendedor(vendedor: string, extraFiltros?: FiltrosGlobais) {
  const { clause: extraClause, inputs: extraInputs } = extraFiltros ? buildWhere({ ...extraFiltros, vendedores: [] }) : { clause: '', inputs: [] }
  const inputs = [{ name: 'vend', type: sql.NVarChar(100), value: vendedor }, ...extraInputs]
  const whereBase = 'WHERE fv.nome_vendedor = @vend'
  const clause = extraClause ? whereBase + ' AND ' + extraClause.replace('WHERE ', '') : whereBase
  return query<{ codParc: number; razaoSocial: string; perfilParceiro: string; uf: string; faturamento: number; volume: number; ultimaCompra: string }>(`
...
export async function getEvolucaoConsolidada(f: FiltrosGlobais) {
  const { clause, inputs } = buildWhere(f)
  return query<{ mes: string; faturamento: number; volume: number; clientes: number }>(`
    SELECT FORMAT(fv.dt_entrega_cliente,'yyyy-MM') AS mes,
      COALESCE(SUM(fv.valor_pendente),0) AS faturamento,
...
export async function getVendedoresEvolucaoPorTipo(f: FiltrosGlobais) {
  const { clause, inputs } = buildWhere({ ...f, tiposReceita: undefined })
  return query<{ mes: string; tipoReceita: string; faturamento: number }>(`
    SELECT
...
export async function getClientesConsolidados(f: FiltrosGlobais) {
  const { clause, inputs } = buildWhere(f)
  return query<{ codParc: number; razaoSocial: string; perfilParceiro: string; uf: string; faturamento: number; volume: number; ultimaCompra: string }>(`
    SELECT fv.cod_parc AS codParc, COALESCE(dc.razao_social, fv.RAZAOSOCIAL) AS razaoSocial, fv.perfil_parceiro AS perfilParceiro,
      fv.uf,
...
// ─── Clientes ─────────────────────────────────────────────────────────────────
export async function getListaClientes(f: FiltrosGlobais) {
  const { clause, inputs } = buildWhere(f)
  // gd usa datas GLOBAIS (sem filtro de período) para calcular status real do cliente
  return query<{ codParc: number; razaoSocial: string; perfilParceiro: string; nomeVendedor: string; uf: string; faturamento: number; volume: number; produtos: number; pedidos: number; ultimaCompra: string; primeiraCompra: string }>(`
    WITH gd AS (
      SELECT cod_parc,
...
export async function getHistoricoCliente(codParc: number, filtros?: FiltrosGlobais) {
  const extra = filtros ? buildWhere({ ...filtros, codParc: undefined }) : { clause: '', inputs: [] }
  const inputs = [{ name: 'codParc', type: sql.Int, value: codParc }, ...extra.inputs]
  const addCond = extra.clause ? extra.clause.replace('WHERE ', ' AND ') : ''
  return query<{ mes: string; faturamento: number; volume: number; produtos: number; tipoReceita: string }>(`
    SELECT FORMAT(fv.dt_entrega_cliente,'yyyy-MM') AS mes,
      COALESCE(SUM(fv.valor_pendente),0) AS faturamento, COALESCE(SUM(fv.qtd_pendente_kg),0) AS volume,
...
export async function getMixCliente(codParc: number, filtros?: FiltrosGlobais) {
  const extra = filtros ? buildWhere({ ...filtros, codParc: undefined }) : { clause: '', inputs: [] }
  const inputs = [{ name: 'codParc', type: sql.Int, value: codParc }, ...extra.inputs]
  const addCond = extra.clause ? extra.clause.replace('WHERE ', ' AND ') : ''
  return query<{ codProduto: number; nomeProduto: string; grupoProduto: string; faturamento: number; volume: number; pedidos: number; ultimaCompra: string }>(`
...
export async function getSaudeCarteira(f: FiltrosGlobais) {
  const { clause, inputs } = buildWhere(f)
  ...
  const rows = await query<{ codParc: number; ultimaCompra: string; primeiraCompra: string }>(`
    WITH gd AS (
      SELECT cod_parc,
        MAX(CONVERT(VARCHAR, dt_entrega_cliente, 23)) AS ultimaCompra,
        MIN(CONVERT(VARCHAR, dt_entrega_cliente, 23)) AS primeiraCompra
  ...
  for (const r of rows) {
    ...
    if (diasPrimeira <= 60)  novos++
    else if (diasUltima <= 75)  ativos++   // ≤ 75 dias (≈ 2,5 meses) = comprou recentemente
    else if (diasUltima <= 180) emRisco++  // 75-180 dias = em risco de esfriamento
    else inativos++
  }
...
export async function getVendasPorEstado(f: FiltrosGlobais) {
  const { clause, inputs } = buildWhere(f)
  return query<{ uf: string; faturamento: number; volume: number; clientes: number }>(`
...
export async function getClientesCrescimento(f: FiltrosGlobais) {
  const dtIni = f.dataInicio || '2026-01-01'
  const dtFim  = f.dataFim  || '2026-12-31'
  const nonDate: FiltrosGlobais = { ...f, dataInicio: undefined, dataFim: undefined, codParc: undefined }
  const { clause: baseClause, inputs: baseInputs } = buildWhere(nonDate)
  const addCond = baseClause ? baseClause.replace(/^WHERE\s+/i, ' AND ') : ''
  const inputs: SqlInput[] = [
    { name: 'dtIni', type: sql.Date, value: dtIni },
    { name: 'dtFim',  type: sql.Date, value: dtFim  },
    ...baseInputs,
  ]
  // effEnd = menor entre @dtFim e hoje — evita comparar com dados futuros inexistentes
  return query<{ codParc: number; razaoSocial: string; perfilParceiro: string; fatAtual: number; fatAnterior: number; crescimentoPct: number }>(`
    WITH eff AS (
...
// ─── Produtos ─────────────────────────────────────────────────────────────────
export async function getPerformanceProdutos(f: FiltrosGlobais) {
  const { clause, inputs } = buildWhere(f)
  return query<{ codProduto: number; nomeProduto: string; grupoProduto: string; faturamento: number; volume: number; clientes: number; pedidos: number }>(`
    SELECT fv.cod_produto AS codProduto, COALESCE(dp.nome_produto, fv.nome_produto) AS nomeProduto, fv.grupo_produto AS grupoProduto,
      COALESCE(SUM(fv.valor_pendente),0) AS faturamento, COALESCE(SUM(fv.qtd_pendente_kg),0) AS volume,
...
export async function getEvolucaoProduto(codProduto: number, f: FiltrosGlobais) {
  const parts: string[] = ['fv.cod_produto = @codProduto', 'fv.flag_devolucao = 0', '(fv.cod_top IS NULL OR fv.cod_top != 1023)', "(fv.[top] IS NULL OR fv.[top] NOT LIKE '%ESTOQUE MINIM%')"]
  const inputs: SqlInput[] = [{ name: 'codProduto', type: sql.BigInt, value: codProduto }]
  if (f.dataInicio) { inputs.push({ name: 'dtIni', type: sql.Date, value: f.dataInicio }); parts.push('fv.dt_entrega_cliente >= @dtIni') }
  if (f.dataFim) { inputs.push({ name: 'dtFim', type: sql.Date, value: f.dataFim }); parts.push('fv.dt_entrega_cliente <= @dtFim') }
  return query<{ mes: string; faturamento: number; volume: number; clientes: number }>(`
    SELECT FORMAT(fv.dt_entrega_cliente,'yyyy-MM') AS mes,
...
export async function getEvolucaoProdutosConsolidada(f: FiltrosGlobais) {
  return getEvolucaoConsolidada(f)
}

// ─── Segmentos ────────────────────────────────────────────────────────────────
export async function getPerformanceSegmentos(f: FiltrosGlobais) {
  const { clause, inputs } = buildWhere(f)
  return query<{ segmento: string; faturamento: number; volume: number; clientes: number; produtos: number }>(`
...
export async function getEvolucaoSegmento(segmento: string) {
  return query<{ mes: string; faturamento: number; volume: number }>(`
    SELECT FORMAT(fv.dt_entrega_cliente,'yyyy-MM') AS mes,
      COALESCE(SUM(fv.valor_pendente),0) AS faturamento, COALESCE(SUM(fv.qtd_pendente_kg),0) AS volume
    ...
    GROUP BY FORMAT(fv.dt_entrega_cliente,'yyyy-MM')
    ORDER BY FORMAT(fv.dt_entrega_cliente,'yyyy-MM')
  `, [{ name: 'seg', type: sql.NVarChar(100), value: segmento }])
}

// ─── Pipeline ─────────────────────────────────────────────────────────────────
export async function getPipelineComercial(f: FiltrosGlobais) {
  const { clause, inputs } = buildWhere(f)
  return query<{ tipoReceita: string; faturamento: number; volume: number; clientes: number; registros: number }>(`
    SELECT
      CASE WHEN fv.tipo_receita = 'DEVOLUCAO' THEN 'VENDA_FIRME' ELSE fv.tipo_receita END AS tipoReceita,
...
export async function getPipelineDetalhado(f: FiltrosGlobais) {
  const { clause, inputs } = buildWhere(f)
  return query<{ tipoReceita: string; projeto: string; codParc: number; razaoSocial: string; nomeVendedor: string; faturamento: number; volume: number; dtPrevista: string }>(`
...
export function normalizeSellerName(name: string): string {
  const base = name
    ...
    .trim()
    .replace(/\s+/g, ' ')
  return SELLER_ALIASES[base] ?? base
}

// Gera CASE WHEN SQL para aplicar aliases sobre uma expressão normalizada
function sqlApplyAliases(expr: string): string {
  const entries = Object.entries(SELLER_ALIASES)
...
export async function getFaturamentoCrmMapping() {
  return query<{
...
function pipelineWhereClause(col: string, ids?: number[]): string {
  if (ids && ids.length > 0) {
    const safe = ids.filter(id => !PIPELINE_BLACKLIST.includes(id))
    if (safe.length > 0) return `AND ${col} IN (${safe.join(',')})`
  }
  return `AND ${col} NOT IN (${PIPELINE_BLACKLIST.join(',')})`
}

function userWhereClause(col: string, userId?: number): string {
  return userId != null ? `AND ${col} = ${userId}` : ''
}

export async function getCrmKpisPorVendedor() {
  const bl = PIPELINE_BLACKLIST.join(',')
  return query<{
    crmUserId: number
    emAndamento: number; valorAndamento: number
    ganhos: number;      valorGanho: number
    perdidos: number;    taxaConversao: number; cicloGanhos: number
  }>(`
    SELECT
...
export async function getCrmVendedores() {
...
export async function getFunilKpis(pipelineIds?: number[], userId?: number) {
  ...
  return query<{
    emAndamento: number; valorPipeline: number
    ganhos: number; valorGanho: number
    perdidos: number; taxaConversao: number; diasMedioFechamento: number
  }>(`
    SELECT
      SUM(CASE WHEN stage_semantic_id = 'P' THEN 1 ELSE 0 END) AS emAndamento,
...
export async function getFunilPorEtapa(pipelineIds?: number[], userId?: number) {
  ...
  return query<{ etapa: string; pipeline: string; total: number; valorTotal: number; semantic: string; pipelineId: number; stageId: string }>(`
    SELECT
      ds.name AS etapa,
      p.name  AS pipeline,
      COUNT(*) AS total,
...
export async function getFunilPorPipeline(pipelineIds?: number[], userId?: number) {
  const filter = pipelineWhereClause('CAST(COALESCE(d.category_id, \'0\') AS INT)', pipelineIds)
  const userF  = userWhereClause('d.assigned_by_id', userId)
  return query<{ pipeline: string; emAndamento: number; ganhos: number; perdidos: number; valorPipeline: number; taxaConversao: number }>(`
    SELECT
...
export async function getFunilTopVendedores(pipelineIds?: number[], userId?: number) {
  const filter = pipelineWhereClause('CAST(COALESCE(d.category_id, \'0\') AS INT)', pipelineIds)
  const userF  = userWhereClause('d.assigned_by_id', userId)
  return query<{ nome: string; emAndamento: number; ganhos: number; perdidos: number; valorPipeline: number; taxaConversao: number }>(`
    SELECT TOP 10
...
export async function getFunilEvolucaoMensal(pipelineIds?: number[], userId?: number) {
  const filter = pipelineWhereClause('CAST(COALESCE(category_id, \'0\') AS INT)', pipelineIds)
  const userF  = userWhereClause('assigned_by_id', userId)
  return query<{ mes: string; abertos: number; ganhos: number; perdidos: number }>(`
    SELECT
...
type Visao = 'calendario' | 'coorte'
type Origem = 'leads' | 'base' | 'total'

function origemClause(origem: Origem): string {
  if (origem === 'leads') return 'AND lead_id IS NOT NULL'
  if (origem === 'base')  return 'AND lead_id IS NULL'
  return ''
}

function pipelineClause(pipelineId: number | null): string {
  if (pipelineId === 31) return "category_id = '31'"
  if (pipelineId === null) return "CAST(COALESCE(category_id,'0') AS INT) NOT IN (15, 23, 25)"
  return "(category_id = '0' OR category_id IS NULL)"
}

export async function getPanoramaLeadsSnapshot() {
  const rows = await query<{ emAndamento: number }>(`
    SELECT COUNT(*) AS emAndamento FROM crm_leads WHERE status_semantic_id = 'P'
  `)
  return rows[0]?.emAndamento ?? 0
}

export async function getPanoramaDealsSnapshot(pipelineId: number | null, origem: Origem, userId?: number) {
  const pip   = pipelineClause(pipelineId)
  const ori   = origemClause(origem)
  const userF = userWhereClause('assigned_by_id', userId)
  const rows = await query<{ emAndamento: number; valorEmAndamento: number }>(`
...
export async function getPanoramaLeads(dateIni: string, dateFim: string, visao: Visao) {
  if (visao === 'calendario') {
    const criados = await query<{ periodo: string; criados: number; comMovimentacao: number }>(`
      SELECT
      ...
      ORDER BY periodo
    `)
    const fechados = await query<{ periodo: string; convertidos: number; perdidos: number; cicloMedio: number | null }>(`
      SELECT
    ...
    const rows = await query<{ periodo: string; criados: number; comMovimentacao: number; emAndamento: number; convertidos: number; perdidos: number; taxaConv: number | null; cicloMedio: number | null }>(`
      SELECT
        FORMAT(TRY_CAST(date_create AS DATE),'yyyy-MM') AS periodo,
        COUNT(*) AS criados,
        SUM(CASE WHEN TRY_CAST(moved_time AS DATE) BETWEEN '${dateIni}' AND '${dateFim}' THEN 1 ELSE 0 END) AS comMovimentacao,
...
export async function getPanoramaDeals(dateIni: string, dateFim: string, visao: Visao, pipelineId: number | null, origem: Origem, userId?: number) {
  const pip   = pipelineClause(pipelineId)
  const ori   = origemClause(origem)
  const userF = userWhereClause('assigned_by_id', userId)

  if (visao === 'calendario') {
    const criados = await query<{ periodo: string; criados: number }>(`
      ...
      ORDER BY periodo
    `)
    const fechados = await query<{ periodo: string; ganhos: number; valorGanhos: number; perdidos: number; cicloTotal: number | null; cicloGanhos: number | null }>(`
      SELECT
    ...
    const rows = await query<{ periodo: string; criados: number; emAndamento: number; valorEmAndamento: number; ganhos: number; valorGanhos: number; perdidos: number; taxaConv: number | null; cicloTotal: number | null; cicloGanhos: number | null }>(`
      SELECT
        FORMAT(TRY_CAST(date_create AS DATE),'yyyy-MM') AS periodo,
        COUNT(*) AS criados,
...
// ─── Representatividade ───────────────────────────────────────────────────────
export async function getClientesRepresentatividade(f: FiltrosGlobais) {
  const { clause, inputs } = buildWhere(f)
  return query<{ codParc: number; razaoSocial: string; faturamento: number; volume: number; produtos: number }>(`
...
export async function getProjetoRepresentatividade(f: FiltrosGlobais, nomeVendedor?: string) {
  const { clause, inputs } = buildWhere(f)
  const vendCond = nomeVendedor ? `AND fv.nome_vendedor = @vendSingle` : ''
  const allInputs = nomeVendedor ? [...inputs, { name: 'vendSingle', type: sql.NVarChar(100), value: nomeVendedor }] : inputs
  return query<{ projeto: string; faturamento: number; volume: number; clientes: number }>(`
...
export async function getDrillDownTipoReceita(tipoReceita: string, f: FiltrosGlobais) {
  const { clause, inputs } = buildWhere({ ...f, tiposReceita: [tipoReceita] })
  return query<{ codParc: number; razaoSocial: string; codProduto: number; nomeProduto: string; grupoProduto: string; nomeVendedor: string; faturamento: number; volume: number; registros: number; dtPrevEntrega: string }>(`
    SELECT fv.cod_parc AS codParc, COALESCE(dc.razao_social, fv.RAZAOSOCIAL) AS razaoSocial,
...
export async function getFiltrosDisponiveis() {
  const p = await getPool()
  if (!p) return { mercados: [], vendedores: [], projetos: [], grupos: [], clientes: [] }
  const [mercados, vendedores, projetos, grupos, clientes] = await Promise.all([
    query<{ value: string }>('SELECT DISTINCT mercado_vendas AS value FROM fato_vendas WHERE mercado_vendas IS NOT NULL ORDER BY mercado_vendas'),
    query<{ value: string }>('SELECT DISTINCT nome_vendedor AS value FROM fato_vendas WHERE nome_vendedor IS NOT NULL ORDER BY nome_vendedor'),
    query<{ value: string }>('SELECT DISTINCT projeto AS value FROM fato_vendas WHERE projeto IS NOT NULL ORDER BY projeto'),
    query<{ value: string }>('SELECT DISTINCT grupo_produto AS value FROM fato_vendas WHERE grupo_produto IS NOT NULL ORDER BY grupo_produto'),
    query<{ codParc: number; razaoSocial: string }>('SELECT cod_parc AS codParc, razao_social AS razaoSocial FROM dim_cliente ORDER BY razao_social'),
  ])
  return {
...
export async function getTotalVendas() {
  const rows = await query<{ count: number }>('SELECT COUNT(*) AS count FROM fato_vendas')
  return Number(rows[0]?.count ?? 0)
}

export async function reclassificarTipoReceita() {
  const p = await getPool()
  if (!p) throw new Error('Sem conexão com banco')
  const req = p.request()
...
// ─── Insights ─────────────────────────────────────────────────────────────────
export async function getInsights(filtro?: { tipo?: string; prioridade?: string; status?: string }) {
  const parts: string[] = []
  const inputs: SqlInput[] = []
  if (filtro?.tipo) { parts.push('tipo_insight = @tipo'); inputs.push({ name: 'tipo', type: sql.NVarChar(50), value: filtro.tipo }) }
  if (filtro?.prioridade) { parts.push('prioridade = @prio'); inputs.push({ name: 'prio', type: sql.NVarChar(10), value: filtro.prioridade }) }
  if (filtro?.status) { parts.push('status = @status'); inputs.push({ name: 'status', type: sql.NVarChar(20), value: filtro.status }) }
...
export async function insertInsight(data: { tipoInsight: string; codParc?: number; nomeCliente?: string; nomeVendedor?: string; codProduto?: number; nomeProduto?: string; segmento?: string; descricao: string; impactoEstimadoR?: number; impactoEstimadoKg?: number; prioridade: string }) {
  const p = await getPool()
  if (!p) return
  await p.request()
    .input('tipo', sql.NVarChar(50), data.tipoInsight)
    .input('codParc', sql.Int, data.codParc ?? null)
    .input('nomeCliente', sql.NVarChar(255), data.nomeCliente ?? null)
    .input('nomeVendedor', sql.NVarChar(100), data.nomeVendedor ?? null)
    .input('codProduto', sql.BigInt, data.codProduto ?? null)
    .input('nomeProduto', sql.NVarChar(255), data.nomeProduto ?? null)
    .input('segmento', sql.NVarChar(100), data.segmento ?? null)
    .input('descricao', sql.NVarChar(sql.MAX), data.descricao)
    .input('impR', sql.Decimal(15, 2), data.impactoEstimadoR ?? null)
    .input('impKg', sql.Decimal(15, 3), data.impactoEstimadoKg ?? null)
    .input('prio', sql.NVarChar(10), data.prioridade)
...
export async function createUpload(nomeArquivo: string, uploadedBy?: number) {
  const p = await getPool()
  if (!p) throw new Error('DB unavailable')
  const res = await p.request()
    .input('nome', sql.NVarChar(255), nomeArquivo)
    .input('by', sql.Int, uploadedBy ?? null)
    .query(`INSERT INTO uploads (nome_arquivo, uploaded_by) OUTPUT INSERTED.id VALUES (@nome, @by)`)
  return res.recordset[0].id as number
}

export async function updateUpload(id: number, data: { status?: string; totalRegistros?: number; registrosImportados?: number; registrosErro?: number; erroMensagem?: string }) {
  const p = await getPool()
  if (!p) return
  const sets: string[] = ['updated_at = GETDATE()']
  const req = p.request().input('id', sql.Int, id)
  if (data.status !== undefined) { req.input('status', sql.NVarChar(20), data.status); sets.push('status = @status') }
  if (data.totalRegistros !== undefined) { req.input('total', sql.Int, data.totalRegistros); sets.push('total_registros = @total') }
  if (data.registrosImportados !== undefined) { req.input('imp', sql.Int, data.registrosImportados); sets.push('registros_importados = @imp') }
  if (data.registrosErro !== undefined) { req.input('err', sql.Int, data.registrosErro); sets.push('registros_erro = @err') }
  if (data.erroMensagem !== undefined) { req.input('msg', sql.NVarChar(sql.MAX), data.erroMensagem); sets.push('erro_mensagem = @msg') }
...
export async function importarVendas(
  registros: Array<{
    nroUnico: number; nroNota?: number | null; tipmov: string; tipoReceita: string
    dtNeg?: string | null; dtPrevEntregaEmbarque?: string | null; dtMov?: string | null
    codParc: number; codProduto: number; nomeVendedor?: string | null
    projeto?: string | null; mercadoVendas?: string | null; grupoProduto?: string | null
    perfilParceiro?: string | null; uf?: string | null; top?: string | null
    codTipop?: number | null
    qtdNegociada?: number | null; qtdPendenteKg?: number | null; valorPendente?: number | null
    valorIcms?: number | null; valorPis?: number | null; valorCofins?: number | null
    vlrSt?: number | null; percDescBonificado?: number | null; flagDevolucao?: boolean
  }>,
  uploadId: number
) {
  const p = await getPool()
  ...
  for (let i = 0; i < registros.length; i += BATCH) {
    ...
    for (const r of lote) {
      await p.request()
        ...
        .input('nroUnico', sql.BigInt, r.nroUnico)
        .input('nroNota', sql.BigInt, r.nroNota ?? null)
        .input('tipmov', sql.NVarChar(1), r.tipmov)
        .input('tipoReceita', sql.NVarChar(20), r.tipoReceita)
        .input('dtNeg', sql.Date, r.dtNeg ?? null)
        .input('dtPrev', sql.Date, r.dtPrevEntregaEmbarque ?? null)
        .input('dtMov', sql.Date, r.dtMov ?? null)
        .input('codParc', sql.Int, r.codParc)
        .input('codProduto', sql.BigInt, r.codProduto)
        .input('nomeVendedor', sql.NVarChar(100), r.nomeVendedor ?? null)
        .input('projeto', sql.NVarChar(100), r.projeto ?? null)
        .input('mercado', sql.NVarChar(100), r.mercadoVendas ?? null)
        .input('grupo', sql.NVarChar(100), r.grupoProduto ?? null)
        .input('perfil', sql.NVarChar(100), r.perfilParceiro ?? null)
        .input('uf', sql.NVarChar(2), r.uf ?? null)
        .input('top', sql.NVarChar(150), r.top ?? null)
        .input('codTop', sql.Int, r.codTipop ?? null)
        .input('qtdNeg', sql.Decimal(15, 3), r.qtdNegociada ?? null)
        .input('qtdPend', sql.Decimal(15, 3), r.qtdPendenteKg ?? null)
        .input('valPend', sql.Decimal(15, 2), r.valorPendente ?? null)
        .input('icms', sql.Decimal(15, 2), r.valorIcms ?? null)
        .input('pis', sql.Decimal(15, 2), r.valorPis ?? null)
        .input('cofins', sql.Decimal(15, 2), r.valorCofins ?? null)
        .input('st', sql.Decimal(15, 2), r.vlrSt ?? null)
        .input('desc', sql.Decimal(5, 2), r.percDescBonificado ?? null)
        .input('flag', sql.Bit, r.flagDevolucao ? 1 : 0)
...
// ─── Tarefas ──────────────────────────────────────────────────────────────────
export async function listTasks(filters?: { status?: string; prioridade?: string }) {
  const parts: string[] = []
  const inputs: SqlInput[] = []
  if (filters?.status) { parts.push('status = @status'); inputs.push({ name: 'status', type: sql.NVarChar(20), value: filters.status }) }
  if (filters?.prioridade) { parts.push('prioridade = @prio'); inputs.push({ name: 'prio', type: sql.NVarChar(10), value: filters.prioridade }) }
  const clause = parts.length ? 'WHERE ' + parts.join(' AND ') : ''
  return query(`SELECT * FROM tasks ${clause} ORDER BY created_at DESC`, inputs)
}

export async function createTask(data: { titulo: string; descricao?: string; status?: string; prioridade?: string; responsavel?: string; prazo?: string; criadoPor?: string; criadoPorId?: number }) {
  const p = await getPool()
  if (!p) throw new Error('DB unavailable')
  const res = await p.request()
    .input('titulo', sql.NVarChar(255), data.titulo)
    .input('desc', sql.NVarChar(sql.MAX), data.descricao ?? null)
    .input('status', sql.NVarChar(20), data.status ?? 'A_FAZER')
    .input('prio', sql.NVarChar(10), data.prioridade ?? 'MEDIA')
    .input('resp', sql.NVarChar(100), data.responsavel ?? null)
    .input('prazo', sql.Date, data.prazo ?? null)
    .input('criado', sql.NVarChar(100), data.criadoPor ?? null)
    .input('criadoId', sql.Int, data.criadoPorId ?? null)
  ...
  return { id: res.recordset[0].id as number }
}

export async function updateTask(id: number, data: Partial<{ titulo: string; descricao: string; status: string; prioridade: string; responsavel: string; prazo: string }>) {
  const p = await getPool()
  if (!p) return
  const sets: string[] = ['updated_at = GETDATE()']
  const req = p.request().input('id', sql.Int, id)
  if (data.titulo !== undefined) { req.input('titulo', sql.NVarChar(255), data.titulo); sets.push('titulo = @titulo') }
  if (data.descricao !== undefined) { req.input('desc', sql.NVarChar(sql.MAX), data.descricao); sets.push('descricao = @desc') }
  if (data.status !== undefined) { req.input('status', sql.NVarChar(20), data.status); sets.push('status = @status') }
  if (data.prioridade !== undefined) { req.input('prio', sql.NVarChar(10), data.prioridade); sets.push('prioridade = @prio') }
  if (data.responsavel !== undefined) { req.input('resp', sql.NVarChar(100), data.responsavel); sets.push('responsavel = @resp') }
  if (data.prazo !== undefined) { req.input('prazo', sql.Date, data.prazo); sets.push('prazo = @prazo') }
...
export async function deleteTask(id: number) {
  const p = await getPool()
  if (!p) return
  await p.request().input('id', sql.Int, id).query('DELETE FROM tasks WHERE id = @id')
  return { success: true }
}

// ─── Chat Messages ────────────────────────────────────────────────────────────
export async function getChatHistory(sessionId: string, limit = 50) {
  return query<{ id: number; session_id: string; role: string; content: string; created_at: Date }>(
    `SELECT TOP ${limit} id, session_id, role, content, created_at FROM chat_messages WHERE session_id = @sid ORDER BY created_at ASC`,
    [{ name: 'sid', type: sql.NVarChar(64), value: sessionId }]
  )
}

export async function saveChatMessage(sessionId: string, userId: number | null, role: string, content: string) {
  const p = await getPool()
  if (!p) return
  await p.request()
    .input('sid', sql.NVarChar(64), sessionId)
...
export async function clearChatHistory(sessionId: string) {
  const p = await getPool()
  if (!p) return
  await p.request().input('sid', sql.NVarChar(64), sessionId).query('DELETE FROM chat_messages WHERE session_id = @sid')
}

export async function getChatSessions(userId: number) {
  return query<{ session_id: string; title: string | null; last_at: Date; message_count: number }>(
    `SELECT m.session_id,
...
export async function getDashboardVendedores(f: FiltrosGlobais) {
  const { clause, inputs } = buildWhere(f)
  return query<{ nomeVendedor: string; faturamento: number; volume: number; clientes: number }>(`
    SELECT fv.nome_vendedor AS nomeVendedor,
      COALESCE(SUM(fv.valor_pendente),0) AS faturamento,
      COALESCE(SUM(fv.qtd_pendente_kg),0) AS volume,
      COUNT(DISTINCT fv.cod_parc) AS clientes
...
export async function getDashboardSegmentos(f: FiltrosGlobais) {
  return getPerformanceSegmentos(f)
}

export async function getDashboardTopClientes(f: FiltrosGlobais) {
  const { clause, inputs } = buildWhere(f)
  return query<{ codParc: number; razaoSocial: string; faturamento: number; volume: number; produtos: number }>(`
    SELECT fv.cod_parc AS codParc, COALESCE(dc.razao_social, fv.RAZAOSOCIAL) AS razaoSocial,
      COALESCE(SUM(fv.valor_pendente),0) AS faturamento, COALESCE(SUM(fv.qtd_pendente_kg),0) AS volume,
...
function buildNpWhere(f: FiltrosGlobais): { clause: string; inputs: SqlInput[] } {
...
export async function getNovoProjetosKpisV2(f: FiltrosGlobais, modoCard?: 'abertos' | 'totais') {
  const { clause, inputs } = buildNpWhere(f)
  const dParams: SqlInput[] = []
  if (f.dataInicio) dParams.push({ name: 'pDtIni', type: sql.Date, value: f.dataInicio })
  if (f.dataFim)    dParams.push({ name: 'pDtFim', type: sql.Date, value: f.dataFim })
  ...
  const [r1, r2, r2b, r3] = await Promise.all([
    ...
    query<{ total: number; convertidos: number }>(`
      WITH primeiros AS (
        SELECT cod_parc, cod_produto, MIN(dt_entrega_cliente) AS dt_primeiro
        FROM fato_vendas
        WHERE projeto IN ('NOVOS PROJETOS', 'TESTE INDUSTRIAL')
...
export async function getNovoProjetosPorMesV2(f: FiltrosGlobais, modoCard?: 'abertos' | 'totais') {
  const { clause, inputs } = buildNpWhere(f)
  const dParams: SqlInput[] = []
  if (f.dataInicio) dParams.push({ name: 'pDtIni', type: sql.Date, value: f.dataInicio })
  if (f.dataFim)    dParams.push({ name: 'pDtFim', type: sql.Date, value: f.dataFim })
  ...
  return query<{ mes: string; projetos: number; faturamento: number }>(`
    ${NP_PRIMEIROS_SQL}
    SELECT
      FORMAT(fv.dt_entrega_cliente, 'yyyy-MM') AS mes,
...
export async function getListaNovosProjetos(f: FiltrosGlobais, modoCard?: 'abertos' | 'totais') {
  const { clause, inputs } = buildNpWhere(f)
  const dParams: SqlInput[] = []
  if (f.dataInicio) dParams.push({ name: 'pDtIni', type: sql.Date, value: f.dataInicio })
  if (f.dataFim)    dParams.push({ name: 'pDtFim', type: sql.Date, value: f.dataFim })
...
export async function getNovoProjetosDrilldown(mes: string, f: FiltrosGlobais) {
  const fNoDate: FiltrosGlobais = { ...f, dataInicio: undefined, dataFim: undefined }
  const { clause, inputs } = buildNpWhere(fNoDate)
  const allInputs: SqlInput[] = [...inputs, { name: 'npMes', type: sql.NVarChar(7), value: mes }]
...
// ─── CRUD Metas 2026 ─────────────────────────────────────────────────────────
export async function listMetasAdmin() {
  return query<{ id: number; nomeVendedor: string; mes: string; valorMeta: number; projeto: string | null }>(`
...
export async function upsertMeta(data: { nomeVendedor: string; mes: string; valorMeta: number; projeto: string | null }) {
  const p = await getPool()
  if (!p) throw new Error('Sem conexão com o banco')
  const req = p.request()
    .input('vend',   sql.NVarChar(200),  data.nomeVendedor)
    .input('mes',    sql.NVarChar(7),    data.mes)
    .input('meta',   sql.Decimal(15,2),  data.valorMeta)
    .input('proj',   sql.NVarChar(100),  data.projeto ?? null)
  await req.query(`
...
export async function deleteMeta(id: number) {
  const p = await getPool()
  if (!p) throw new Error('Sem conexão com o banco')
  await p.request().input('id', sql.Int, id).query(`DELETE FROM metas_2026 WHERE id = @id`)
}

export async function getMetasVendedores(f?: FiltrosGlobais) {
  const parts: string[] = []
  const inputs: SqlInput[] = []
  if (f?.projetos?.length) {
    parts.push(`projeto IN (${f.projetos.map((_, i) => `@proj${i}`).join(',')})`)
    f.projetos.forEach((p, i) => inputs.push({ name: `proj${i}`, type: sql.NVarChar(100), value: p }))
  }
  if (f?.mercados?.length) {
    parts.push(`mercado_vendas IN (${f.mercados.map((_, i) => `@merc${i}`).join(',')})`)
    f.mercados.forEach((m, i) => inputs.push({ name: `merc${i}`, type: sql.NVarChar(100), value: m }))
  }
  if (f?.vendedores?.length) {
    parts.push(`nome_vendedor IN (${f.vendedores.map((_, i) => `@vend${i}`).join(',')})`)
    f.vendedores.forEach((v, i) => inputs.push({ name: `vend${i}`, type: sql.NVarChar(200), value: v }))
  }
  const clause = parts.length ? 'WHERE ' + parts.join(' AND ') : ''
  return query<{ nomeVendedor: string; mes: string; valorMeta: number; projeto: string; mercadoVendas: string }>(`
...
export async function createForecastSnapshot(): Promise<{ inserted: number; snapshotDate: string }> {
  const p = await getPool()
  if (!p) throw new Error('Database unavailable')
  const snapshotDate = new Date().toISOString().split('T')[0]
  const result = await p.request().input('snapDate', sql.Date, snapshotDate).query(`
...
export async function getSnapshotDates() {
  return query<{ snapshotDate: string; totalRows: number }>(`
...
function buildSnapshotWhere(snapshotDate: string, f: FiltrosGlobais): { clause: string; inputs: SqlInput[] } {
  const parts: string[] = [`CONVERT(VARCHAR(10), fs.snapshot_date, 23) = @snapDate`]
  const inputs: SqlInput[] = [{ name: 'snapDate', type: sql.NVarChar(10), value: snapshotDate }]
  if (f.dataInicio) { inputs.push({ name: 'sDtIni', type: sql.Date, value: f.dataInicio }); parts.push('fs.dt_entrega_cliente >= @sDtIni') }
  if (f.dataFim)    { inputs.push({ name: 'sDtFim', type: sql.Date, value: f.dataFim });    parts.push('fs.dt_entrega_cliente <= @sDtFim') }
  if (f.mercados?.length)      { f.mercados.forEach((v,i)      => inputs.push({ name:`smrc${i}`, type:sql.NVarChar(100), value:v }));  parts.push(`fs.mercado_vendas IN (${f.mercados.map((_,i)=>`@smrc${i}`).join(',')})`) }
  if (f.vendedores?.length)    { f.vendedores.forEach((v,i)    => inputs.push({ name:`svnd${i}`, type:sql.NVarChar(100), value:v }));  parts.push(`fs.nome_vendedor IN (${f.vendedores.map((_,i)=>`@svnd${i}`).join(',')})`) }
  if (f.projetos?.length)      { f.projetos.forEach((v,i)      => inputs.push({ name:`sprj${i}`, type:sql.NVarChar(100), value:v }));  parts.push(`fs.projeto IN (${f.projetos.map((_,i)=>`@sprj${i}`).join(',')})`) }
  if (f.gruposProduto?.length) { f.gruposProduto.forEach((v,i) => inputs.push({ name:`sgrp${i}`, type:sql.NVarChar(100), value:v }));  parts.push(`fs.grupo_produto IN (${f.gruposProduto.map((_,i)=>`@sgrp${i}`).join(',')})`) }
  if (f.tiposReceita?.length) {
    const tipos = f.tiposReceita.includes('VENDA_FIRME') ? [...new Set([...f.tiposReceita, 'DEVOLUCAO'])] : f.tiposReceita
    tipos.forEach((v,i) => inputs.push({ name:`strc${i}`, type:sql.NVarChar(50), value:v }))
    parts.push(`fs.tipo_receita IN (${tipos.map((_,i)=>`@strc${i}`).join(',')})`)
  }
  return { clause: 'WHERE ' + parts.join(' AND '), inputs }
}

export async function getSnapshotComparativo(f: FiltrosGlobais) {
  const dates = await getSnapshotDates()
  if (!dates.length) return { snapshotDate: null, rows: [] as Array<{
    codParc: number; razaoSocial: string;
    snapValor: number; currValor: number; diffValor: number;
    snapVolume: number; currVolume: number; dtEntrega: string | null
  }> }
  ...
  const [snapRows, currRows] = await Promise.all([
    query<{ codParc: number; razaoSocial: string; valor: number; volume: number; dtEntrega: string | null }>(`
...
export async function getSnapshotComparativoProdutos(codParc: number, f: FiltrosGlobais) {
  const dates = await getSnapshotDates()
  if (!dates.length) return { snapshotDate: null, rows: [] as Array<{
    codProduto: number; nomeProduto: string;
    snapValor: number; currValor: number; diffValor: number;
  ...
  const [snapRows, currRows] = await Promise.all([
    query<{ codProduto: number; nomeProduto: string; valor: number; volume: number; dtEntrega: string | null }>(`
...
function buildSnapHistWhere(f: FiltrosGlobais): { clause: string; inputs: SqlInput[] } {
  const parts: string[] = []
  const inputs: SqlInput[] = []
  if (f.dataInicio) { inputs.push({ name: 'shDtIni', type: sql.Date, value: f.dataInicio }); parts.push('fs.dt_entrega_cliente >= @shDtIni') }
  if (f.dataFim)    { inputs.push({ name: 'shDtFim', type: sql.Date, value: f.dataFim });    parts.push('fs.dt_entrega_cliente <= @shDtFim') }
  if (f.mercados?.length)      { f.mercados.forEach((v,i)      => inputs.push({ name:`shm${i}`, type:sql.NVarChar(100), value:v })); parts.push(`fs.mercado_vendas IN (${f.mercados.map((_,i)=>`@shm${i}`).join(',')})`) }
  if (f.vendedores?.length)    { f.vendedores.forEach((v,i)    => inputs.push({ name:`shv${i}`, type:sql.NVarChar(100), value:v })); parts.push(`fs.nome_vendedor IN (${f.vendedores.map((_,i)=>`@shv${i}`).join(',')})`) }
  if (f.projetos?.length)      { f.projetos.forEach((v,i)      => inputs.push({ name:`shp${i}`, type:sql.NVarChar(100), value:v })); parts.push(`fs.projeto IN (${f.projetos.map((_,i)=>`@shp${i}`).join(',')})`) }
  if (f.gruposProduto?.length) { f.gruposProduto.forEach((v,i) => inputs.push({ name:`shg${i}`, type:sql.NVarChar(100), value:v })); parts.push(`fs.grupo_produto IN (${f.gruposProduto.map((_,i)=>`@shg${i}`).join(',')})`) }
  if (f.tiposReceita?.length) {
    const tipos = f.tiposReceita.includes('VENDA_FIRME') ? [...new Set([...f.tiposReceita, 'DEVOLUCAO'])] : f.tiposReceita
    tipos.forEach((v,i) => inputs.push({ name:`sht${i}`, type:sql.NVarChar(50), value:v }))
    parts.push(`fs.tipo_receita IN (${tipos.map((_,i)=>`@sht${i}`).join(',')})`)
  }
  if (f.codParc) { inputs.push({ name: 'shCodParc', type: sql.Int, value: f.codParc }); parts.push('fs.cod_parc = @shCodParc') }
  return { clause: parts.length ? 'WHERE ' + parts.join(' AND ') : '', inputs }
}

export async function getSnapshotHistorico(f: FiltrosGlobais) {
  const { clause: snapClause, inputs: snapInputs } = buildSnapHistWhere(f)
  const { clause: currClause, inputs: currInputs } = buildWhere(f)

  const [snapRows, currRows] = await Promise.all([
    query<{ codParc: number; razaoSocial: string; snapshotDate: string; valor: number; volume: number }>(`
      SELECT fs.cod_parc AS codParc, MAX(fs.razao_social) AS razaoSocial,
  ...
  const snapByParc = new Map<number, { razao: string; byDate: Record<string, { valor: number; volume: number }> }>()
  for (const r of snapRows) {
    if (!snapByParc.has(r.codParc)) snapByParc.set(r.codParc, { razao: r.razaoSocial, byDate: {} })
  ...
  const rows = Array.from(allParc).map(codParc => {
    ...
    return {
      ...
      snapshots: snap?.byDate ?? {} as Record<string, { valor: number; volume: number }>,
...
export async function getSnapshotHistoricoProdutos(codParc: number, f: FiltrosGlobais) {
  const { clause: snapClause, inputs: snapInputs } = buildSnapHistWhere({ ...f, codParc })
  const { clause: currClause, inputs: currInputs } = buildWhere({ ...f, codParc })

  const [snapRows, currRows] = await Promise.all([
    query<{ codProduto: number; nomeProduto: string; snapshotDate: string; valor: number; volume: number }>(`
      SELECT fs.cod_produto AS codProduto, MAX(fs.nome_produto) AS nomeProduto,
        CONVERT(VARCHAR(10), fs.snapshot_date, 23) AS snapshotDate,
        COALESCE(SUM(fs.valor_pendente), 0) AS valor,
        COALESCE(SUM(fs.qtd_pendente_kg), 0) AS volume
  ...
  const allDates = [...new Set(snapRows.map(r => r.snapshotDate))].sort()
  const snapByProd = new Map<number, { nome: string; byDate: Record<string, { valor: number; volume: number }> }>()
  for (const r of snapRows) {
    if (!snapByProd.has(r.codProduto)) snapByProd.set(r.codProduto, { nome: r.nomeProduto, byDate: {} })
...
function buildHistoricoWhere(f: FiltrosHistorico, alias = 'fv'): { clause: string; inputs: SqlInput[] } {
  const parts: string[] = [
  ...
  if (anos.length === 1) {
    inputs.push({ name: 'hAno0', type: sql.Int, value: anos[0] })
    parts.push(`YEAR(${alias}.dt_entrega_cliente) = @hAno0`)
  } else {
    const ps = anos.map((a, i) => { inputs.push({ name: `hAno${i}`, type: sql.Int, value: a }); return `@hAno${i}` })
    parts.push(`YEAR(${alias}.dt_entrega_cliente) IN (${ps.join(',')})`)
  }

  if (f.meses?.length) {
    if (f.meses.length === 1) {
      inputs.push({ name: 'hMes0', type: sql.Int, value: f.meses[0] })
      parts.push(`MONTH(${alias}.dt_entrega_cliente) = @hMes0`)
    } else {
      const ps = f.meses.map((m, i) => { inputs.push({ name: `hMes${i}`, type: sql.Int, value: m }); return `@hMes${i}` })
  ...
  if (f.codParcs?.length) {
    if (f.codParcs.length === 1) {
      inputs.push({ name: 'hParc0', type: sql.Int, value: f.codParcs[0] })
      parts.push(`${alias}.cod_parc = @hParc0`)
    } else {
      const ps = f.codParcs.map((c, i) => { inputs.push({ name: `hParc${i}`, type: sql.Int, value: c }); return `@hParc${i}` })
  ...
  const addIn = (col: string, vals: string[], prefix: string) => {
    if (!vals?.length) return
    const params = vals.map((v, i) => {
      inputs.push({ name: `${prefix}${i}`, type: sql.NVarChar(100), value: v })
      return `@${prefix}${i}`
    })
    parts.push(`${alias}.${col} IN (${params.join(',')})`)
  }
...
export async function getHistoricoFiltros() {
  const [anos, clientes, mercados, grupos, vendedores] = await Promise.all([
    query<{ ano: number }>(`
      ...
      ORDER BY ano DESC
    `),
    query<{ codParc: number; razaoSocial: string }>(`
...
export async function getHistoricoKpis(f: FiltrosHistorico) {
  const { clause, inputs } = buildHistoricoWhere(f)
  const baseF: FiltrosHistorico = { anos: f.anos, meses: f.meses, mercados: f.mercados, gruposProduto: f.gruposProduto, vendedores: f.vendedores }
  const { clause: baseClause, inputs: baseInputs } = buildHistoricoWhere(baseF)

  const [main, base] = await Promise.all([
    query<{ totalValor: number; totalVolume: number; precoMedio: number; qtdProdutos: number; qtdClientes: number }>(`
      SELECT
      ...
      FROM fato_vendas fv ${clause}
    `, inputs),
    query<{ totalValor: number; totalVolume: number }>(`
...
export async function getHistoricoListaClientes(f: FiltrosHistorico) {
  const { clause, inputs } = buildHistoricoWhere(f)
  const rows = await query<{ codParc: number; razaoSocial: string; valor: number; volume: number; precoMedio: number; qtdProdutos: number }>(`
    SELECT
      fv.cod_parc AS codParc,
      COALESCE(MAX(dc.razao_social), MAX(fv.RAZAOSOCIAL)) AS razaoSocial,
      COALESCE(SUM(fv.valor_pendente), 0) AS valor,
...
export async function getHistoricoEvolucaoMensal(f: FiltrosHistorico) {
  const { clause, inputs } = buildHistoricoWhere(f)
  return query<{ mes: number; volume: number; precoMedio: number; valor: number }>(`
    SELECT
...
export async function getHistoricoTopProdutos(f: FiltrosHistorico) {
  const { clause, inputs } = buildHistoricoWhere(f)
  return query<{ codProduto: number; nomeProduto: string; volume: number; valor: number }>(`
    SELECT TOP 15
...
export async function getHistoricoPorEstado(f: FiltrosHistorico) {
  const { clause, inputs } = buildHistoricoWhere(f)
  const rows = await query<{ uf: string; valor: number; volume: number }>(`
...
export async function getHistoricoPorSegmento(f: FiltrosHistorico) {
  const { clause, inputs } = buildHistoricoWhere(f)
  const rows = await query<{ segmento: string; valor: number; volume: number }>(`
...
export async function getHistoricoClienteProdutos(f: FiltrosHistorico) {
  const { clause, inputs } = buildHistoricoWhere(f)
  const rows = await query<{
    codProduto: string
