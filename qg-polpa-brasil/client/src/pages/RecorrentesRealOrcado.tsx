  ...
  return (
    <th onClick={() => onSort(col)}
      className={`px-2 py-2.5 whitespace-nowrap cursor-pointer select-none transition-colors text-[10px] font-semibold uppercase tracking-wider
        ${active ? 'text-foreground' : 'text-slate-400 hover:text-slate-200'}
...
export default function RecorrentesRealOrcado() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-bold text-foreground">Recorrentes — Real x Orçado</h1>
          ...
          <table className="w-full text-xs min-w-[1120px]">
            <thead className="sticky top-0 z-20 bg-slate-900">
              ...
              {!tabelaLoad && sortedData.length > 0 && (() => {
                ...
                return (
                  <tr className="sticky top-[38px] z-20 border-b-2 border-slate-600 bg-slate-900/95 font-semibold text-foreground text-xs">
                ...
                return (
                  <Fragment key={row.codParc}>
                    <tr onClick={() => toggle(row.codParc)}
                      ...
                      <td className="px-3 py-2 text-foreground font-semibold min-w-[200px] max-w-[240px]">
