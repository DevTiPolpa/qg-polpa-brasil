  ...
  return (
    <div className="space-y-4">
      ...
      {/* Filtros — mesmo padrão visual do FiltrosGlobais (Dashboard e demais telas),
          adaptado às opções desta tela (sem período/mercado, que não fazem sentido
          aqui: Tarefas não é uma série temporal de vendas). */}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {temFiltroLinha && (
            <span className="ml-1 px-2 py-0.5 rounded-full bg-primary/15 text-primary text-[11px] font-medium">
              {ORIGEM_LABEL[origemFiltro as ApiTask["origem"]]}
              {codParcFiltro != null && <span> · Cliente {codParcFiltro}</span>}
              {codProdutoFiltro != null && <span> · Produto {codProdutoFiltro}</span>}
            </span>
          )}
          {(temFiltroLinha || responsaveisFiltro.length > 0) && (
            <Button
              variant="ghost"
              size="sm"
              onClick={() => { setResponsaveisFiltro([]); navigate("/tarefas"); }}
