    ...
    <div className="space-y-6 max-w-4xl">
        ...
        <div className="bg-amber-900/30 border border-amber-700 rounded-xl p-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-1.5 bg-slate-700 hover:bg-slate-600 text-foreground text-sm rounded-lg px-3 py-2 transition"
            >
              {copied ? <Check size={14} className="text-green-400" /> : <Copy size={14} />}
              {copied ? 'Copiado!' : 'Copiar'}
            </button>
            <button onClick={() => setTempPassword(null)} className="text-slate-400 hover:text-foreground text-xs">Fechar</button>
          </div>
        </div>
      )}
      ...
      {showCreate && (
        <div className="bg-slate-800 border border-slate-700 rounded-xl p-6">
          <h2 className="text-sm font-semibold text-foreground mb-4">Novo Usuário</h2>
          <form onSubmit={handleCreate} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-sm text-foreground placeholder-slate-400 focus:outline-none focus:border-green-500"
                />
              </div>
              <div>
            ...
            <div>
              ...
              <select
                ...
                className="bg-slate-700 border border-slate-600 rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:border-green-500"
              >
            ...
            <div className="flex gap-3">
              ...
              <button type="button" onClick={() => setShowCreate(false)} className="text-slate-400 hover:text-foreground text-sm transition">Cancelar</button>
            </div>
          </form>
        </div>
      )}
                ...
                <td className="px-4 py-3.5 text-right">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-lg text-slate-400 hover:text-foreground hover:bg-slate-700 disabled:opacity-50 transition"
                    >
                    ...
                    <select
                      ...
                      className="bg-slate-700 border border-slate-600 rounded text-xs text-foreground px-1.5 py-1 disabled:opacity-50 focus:outline-none"
                    >
